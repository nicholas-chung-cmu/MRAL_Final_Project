...
class Mapper:
    ...
    def update_logodds(self, cell, update):
        ...
        idx = cell.row * self.grid.width + cell.col
        if idx < len(self.grid.data):
            self.grid.data[idx] = max(self.grid.min_clamp, min(self.grid.max_clamp, self.grid.data[idx] + update))
        return
    ...
    def add_ray(self, ray, max_range):
        ...
        """
        start = ray.o
        end, freeCells = self.observer.observe_along_ray(ray, max_range)
        ...
        if mag < max_range:
            self.update_hit(self.grid.point_to_cell(end))
        else: 
            self.update_miss(self.grid.point_to_cell(end))

        for fc in freeCells:
            self.update_miss(self.grid.point_to_cell(fc))
